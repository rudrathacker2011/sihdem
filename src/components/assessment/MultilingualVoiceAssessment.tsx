...
export function MultilingualVoiceAssessment() {
  ...
  return (
    <div className="rounded-3xl border border-border bg-card p-4 sm:p-6 md:p-8 shadow-sm mb-6">
      {/* Top Header: Progress & Language Selector & Voice Narration */}
      <div className="flex flex-col gap-3.5 sm:flex-row sm:items-center justify-between border-b border-border pb-4">
        <div className="flex items-center gap-2.5">
          <div className="flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-2xl bg-primary/10 text-primary font-bold text-xs sm:text-sm">
            {currentStep}/7
          </div>
          <div>
            <div className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-primary">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-xl border px-2.5 sm:px-3 py-1.5 text-xs font-bold transition ${
              isNarrating
          ...
          {/* Language Switcher */}
          <div className="flex items-center rounded-xl border border-border bg-background p-0.5 sm:p-1 text-xs">
      ...
      {/* Progress Bar */}
      <div className="mt-4 sm:mt-5 h-2 w-full overflow-hidden rounded-full bg-secondary">
        <div
      ...
      <div className="mt-5 sm:mt-6 space-y-1.5">
        <h3 className="font-heading text-base sm:text-xl font-bold text-foreground leading-snug">
          {question.title}
        </h3>
        ...
        {Object.entries(question.options).map(([optId, label]) => {
          ...
          return (
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-2xl border p-3.5 sm:p-4 text-left transition-all ${
                isSelected
      ...
      {/* Footer Navigation */}
      <div className="mt-6 sm:mt-8 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-3 border-t border-border pt-4 sm:pt-5 pb-2">
        <button
          onClick={handleBack}
          ...
          className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-2.5 text-xs font-bold transition ${
            currentStep > 1
              ? "border-border bg-background text-foreground hover:bg-accent"
              : "border-transparent text-muted-foreground/40 cursor-not-allowed opacity-0 sm:opacity-100"
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center justify-center gap-2 rounded-xl px-6 py-3 text-xs font-bold transition-all shadow-md ${
            currentSelected.length > 0
              ? "bg-primary text-primary-foreground hover:scale-[1.01] shadow-primary/25"
